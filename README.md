...
* **上機免手動負擔：** 訓練速度以 range 滑桿（每格 5 BPM，範圍 40–240）調整，運動中晃動也好拖曳；目前未使用 Screen Wake Lock API，螢幕防休眠需依使用者裝置、PWA 或瀏覽器設定處理。
...
* **Key 根音定位點 ON / OFF：** 訓練頁可一鍵關掉「灰色正方形」的常駐顯示，讓指板完全淨空，進階驗證自己是不是真的背下把位；設定會保存於瀏覽器。
* **全畫面一鍵轉橫向：** 首頁全螢幕按鈕旁提供「畫面：直向 / 橫向」切換，純粹用 CSS 把**整個 App（首頁與特訓畫面）**轉 90 度，**不挑機種、不挑瀏覽器、也不看手機實際拿的方向**（iPhone Safari 一樣能用）；拖拉排序在旋轉狀態下也能正常使用，設定會保存於瀏覽器。
* **行動裝置音訊相容：** iPhone 開著實體靜音開關（或戴耳機）時也能正常出聲；並在來電、鎖定螢幕、切到背景後回來時自動恢復音訊。
...
* **七大類和弦字卡：** 在 builder 建立 / 編輯進行時，和弦庫依性質分為「順階三和弦」（`I~vii°`）、「順階七和弦」（`IM7, iim7, V7, viim7b5` 等）、「同主調三和弦」（`i, ii°, bIII, iv, v, bVI, bVII`）、「同主調七和弦」、「副屬七和弦」（Secondary Dominant）、「關聯 II」（Related II）與「裏和弦」（SubV7 / Tritone Substitute），可自由搭配。
* **統一資料模型：** 每張字卡以 `offset / mode / family / label / modeName` 統一描述，新增和弦僅需於 `CHORD_MODES` 增加一筆，指板、音訊與爬音三大引擎即自動支援。
* **清單優先的進行管理（可存、可編、可刪）：** 分頁精簡為「和弦清單 / 管理」。內建預設（`I - IV - V` 等）只作為初次 seed，之後與自建的進行**全部**都可選取、編輯、刪除，選取樣式統一為選中＝綠、未選＝灰。和弦清單末端有「＋ 新增進行」卡，點它進入拖拉 builder 子頁建立新進行，按「保存」回清單並選中（或「取消」放棄）；管理分頁的「編輯」同樣進 builder 修改。**一律先加入清單再使用**：訓練永遠用清單選中項，builder 編輯期間 START 會停用。保存時自動去重——已有相同進行時直接選中、不重複新增。清單保存於瀏覽器。
...
* **三驅動音序模式：** 提供三種可切換的音程特訓模式演算法引擎——
  * **階梯爬升解鎖：** 兩種階梯曲線可選：「和弦基礎」（Stage 1-5：Triad → 7th → 9th → 11th → 13th）與「音階基礎」（Stage 1-3：Triad → Pentatonic → Mode Scale）。
  * **自訂音序器：** 輸入相對音程指令（如 `L5, 1, 2`），後台依當前和弦自動對齊調式。
  * **三和弦琶音：** 選定一組相鄰三弦（`1-3` / `2-4` / `3-5` / `4-6`）與**爬音方向（上升 低→高 / 下降 高→低）**，系統依和弦性質（大 / 小 / 減）為整組進行自動挑選**琴格移動量最小**的三和弦轉回形（Voice Leading），並**盡量不連續使用同一個轉回形**（柔性懲罰，一般進行下相鄰和弦會落在不同轉回形），讓整輪三和弦集中在同一把位、只換轉回形，並以綠色虛線矩形框住；每完成一輪把位自動往高處階梯推進。Prep 隨每個和弦即時刷三和弦（低音→高音）、Train 依所選方向逐音爬升。
