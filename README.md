...
* **三驅動音序模式：** 提供三種可切換的音程特訓模式演算法引擎——
  ...
  * **三和弦琶音：** 選定一組相鄰三弦（`1-3` / `2-4` / `3-5` / `4-6`），系統依和弦性質（大 / 小 / 減）為整組進行自動挑選**琴格移動量最小**的三和弦轉回形（Voice Leading），讓整輪三和弦集中在同一把位、只換轉回形，並以綠色虛線矩形框住；每完成一輪把位自動往高處階梯推進。Prep 隨每個和弦即時刷三和弦（低音→高音）、Train 由低音到高音逐音爬升。
